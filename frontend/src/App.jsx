...
function Icon({ name, size = 20 }) {
  ...
  const paths = {
    ...
    donors: <path d="M16 11c1.66 0 2.99-1.34 2.99-3S17.66 5 16 5c-1.66 0-3 1.34-3 3s1.34 3 3 3zm-8 0c1.66 0 2.99-1.34 2.99-3S9.66 5 8 5C6.34 5 5 6.34 5 8s1.34 3 3 3zm0 2c-2.33 0-7 1.17-7 3.5V19h14v-2.5c0-2.33-4.67-3.5-7-3.5zm8 0c-.29 0-.62.02-.97.05 1.16.84 1.97 1.97 1.97 3.45V19h6v-2.5c0-2.33-4.67-3.5-7-3.5z"/>,
    campaigns: <path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-5 14H7v-2h7v2zm3-4H7v-2h10v2zm0-4H7V7h10v2z"/>,
    ...
    receipts: <path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-5 14H7v-2h7v2zm3-4H7v-2h10v2zm0-4H7V7h10v2z"/>,
    ...
    calendar: <path d="M19 3h-1V1h-2v2H8V1H6v2H5c-1.11 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5V8h14v11zM9 10H7v2h2v-2zm4 0h-2v2h2v-2zm4 0h-2v2h2v-2zm-8 4H7v2h2v-2zm4 0h-2v2h2v-2zm4 0h-2v2h2v-2z"/>,
    ...
    clock: <path d="M12 20a8 8 0 100-16 8 8 0 000 16zm0-18a10 10 0 110 20 10 10 0 010-20zm.5 5H11v6l5.2 3.12.8-1.3-4.5-2.7V7z"/>,
    alert: <path d="M12 2L1 21h22L12 2zm0 5.5L18.7 19H5.3L12 7.5zM11 10h2v5h-2v-5zm0 6h2v2h-2v-2z"/>,
  };
  return (
        ...
        {data.map((bucket, i) => {
          ...
          return (
            <div key={i} className="chart-bar-wrapper">
              ...
              {bucket.value > 0 && (
                <span className="chart-bar-value">{formatCurrency(bucket.value)}</span>
              )}
            </div>
          );
        })}
...
function RecordDonationModal({ token, donors, campaigns, pledges = [], onClose, onSave }) {
  ...
  const donorPledges = useMemo(() => {
    ...
    return pledges.filter(p => p.donor_id === form.donor_id && p.status !== 'fulfilled');
    ...
    <Modal title="Record Donation" onClose={onClose}>
      <form onSubmit={handleSubmit} className="form">
        <div className="form-row">
          <label>Donor *</label>
          <select required value={form.donor_id} onChange={e => setForm({...form, donor_id: e.target.value, pledge_id: ''})}>
          ...
          >
            ...
            {donorPledges.map(p => (
              <option key={p.id} value={p.id}>
                {formatCurrency(p.amount_pledged - p.amount_fulfilled)} outstanding{p.campaign_name ? ` — ${p.campaign_name}` : ''}
              </option>
            ))}
          </select>
          {form.donor_id && donorPledges.length === 0 && (
            <span className="field-hint">This donor has no outstanding pledges.</span>
          )}
        </div>
        <div className="form-row">
          <label>Amount (₱) *</label>
        ...
        <div className="form-row">
          ...
          <select required value={form.payment_channel} onChange={e => setForm({...form, payment_channel: e.target.value})}>
        ...
        <div className="form-row">
          ...
          <input type="text" maxLength={255} value={form.payment_reference} onChange={e => setForm({...form, payment_reference: e.target.value})} />
        </div>
    ...
    <Modal title="Donation Details" onClose={onClose}>
      <div className="detail-grid">
        <div className="detail-item">
          ...
          <span className="detail-value">{donor?.display_name || 'Unknown'}</span>
        </div>
        <div className="detail-item">
        ...
        <div className="detail-item">
          ...
          <span className="detail-value capitalize">{donation.payment_channel}</span>
        </div>
        <div className="detail-item">
          <span className="detail-label">Status</span>
          <span className="detail-value"><StatusPill status={donation.status} /></span>
        </div>
        <div className="detail-item">
        ...
        {donation.pledge_id && (
          <div className="detail-item">
            ...
            <span className="detail-value">
              {pledge ? `${formatCurrency(donation.amount)} of ${formatCurrency(pledge.amount_pledged)}${pledge.campaign_name ? ` — ${pledge.campaign_name}` : ''}` : 'Linked pledge'}
            </span>
          </div>
        )}
...
function DonorDetailModal({ donor, donations, organizations, onClose }) {
  ...
  const orgName = organizations.find(o => o.id === donor.organization_id)?.name || '—';
  const donorDonations = donations.filter(d => d.donor_id === donor.id).sort((a,b) => new Date(b.donation_date) - new Date(a.donation_date));
  const totalDonated = donorDonations.filter(d => d.status === 'confirmed').reduce((acc, d) => acc + Number(d.amount || 0), 0);
    ...
    <Modal title={isEdit ? 'Edit Donor' : 'New Donor'} onClose={onClose}>
      <form onSubmit={handleSubmit} className="form">
        <div className="form-row">
          ...
          <select required value={form.donor_type} onChange={e => setForm({...form, donor_type: e.target.value})}>
        ...
        <div className="form-row">
          ...
          <input
            ...
            autoFocus
          />
          {fieldErrors.display_name && <span className="field-error">{fieldErrors.display_name[0]}</span>}
        </div>
        <div className="form-row">
          <label>Email</label>
          <input
...
const NAV_ITEMS = [
  ...
  { id: 'organizations', label: 'My Organization', icon: 'settings',     disabled: false },
    ...
    <aside className="sidebar">
      ...
      <nav className="sidebar-nav">
        {NAV_ITEMS.map(item => {
          ...
          const badge = item.id === 'donations' && pendingCount > 0 ? pendingCount : null;
          return (
            <button
              key={item.id}
              className={classNames('nav-item', active && 'active', item.disabled && 'disabled')}
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h1>Dashboard</h1>
          <p className="page-subtitle">Overview of your organization's giving activity</p>
        </div>
      </div>
      ...
      <div className="stats-row">
        ...
        <div className="stat-card">
          <div className="stat-icon blue"><Icon name="donors" size={22}/></div>
          <div className="stat-value">{donors.length}</div>
          <div className="stat-label">Total Donors</div>
        </div>
        <div className="stat-card">
          <div className="stat-icon green"><Icon name="receipts" size={22}/></div>
          <div className="stat-value">{formatCurrency(confirmedThisMonth)}</div>
          <div className="stat-label">Total Confirmed (This Month)</div>
        </div>
        <div className="stat-card">
          <div className="stat-icon amber"><Icon name="bell" size={22}/></div>
          ...
          <table className="table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={4} className="empty">No donations recorded yet.</td></tr>
...
/* ============================== donations page ============================== */
function DonationsPage({ token, donors, campaigns, pledges = [], donations, organizations, loading, onRefresh }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (channelFilter) rows = rows.filter(d => d.payment_channel === channelFilter);
    if (search.trim()) {
    ...
    return rows.sort((a, b) => new Date(b.donation_date) - new Date(a.donation_date));
  ...
  const detailPledge = detailDonation?.pledge_id ? pledges.find(p => p.id === detailDonation.pledge_id) : null;
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h1>Donations</h1>
          <p className="page-subtitle">Record and finalize gifts received — the module's core financial entity</p>
        </div>
      ...
      <div className="tabs">
        <button className={classNames('tab', tab === 'all' && 'active')} onClick={() => setTab('all')}>
          All Donations
        </button>
      ...
      <div className="stats-row">
        <div className="stat-card">
          <div className="stat-icon blue"><Icon name="receipts" size={22}/></div>
          <div className="stat-value">{formatCurrency(confirmedThisMonth)}</div>
          <div className="stat-label">Total Confirmed (This Month)</div>
        </div>
        <div className="stat-card">
      ...
      <div className="card">
        <div className="table-toolbar">
          <h3 className="table-title">{tab === 'all' ? 'All Donations' : 'Pending Confirmation'}</h3>
          ...
          <table className="table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} className="empty">No donations found.</td></tr>
              ) : filtered.map(d => {
                const donor = donorMap[d.donor_id];
                const isLoading = actionLoading && actionLoading.startsWith(d.id);
                return (
                  <tr key={d.id}>
                    <td>
                      <div className="donor-cell">
                        <span className="donor-name">{donor?.display_name || 'Unknown'}</span>
                        {organizations.find(o => o.id === donor?.organization_id)?.name && (
                          <span className="donor-org">{organizations.find(o => o.id === donor?.organization_id)?.name}</span>
                        )}
                      </div>
                    </td>
                    <td className="amount">{formatCurrency(d.amount)}</td>
                    ...
                    <td className="actions">
                      <button className="link" onClick={() => setDetailId(d.id)}>View</button>
                      {d.status === 'pending' && (
                        <>
                          <button className="link" disabled={isLoading} onClick={() => handleAction(d.id, 'confirm')}>
                            {actionLoading === d.id + 'confirm' ? '...' : 'Confirm'}
                          </button>
                          <button className="link danger" disabled={isLoading} onClick={() => handleAction(d.id, 'void')}>
                            {actionLoading === d.id + 'void' ? '...' : 'Void'}
                          </button>
                        </>
                      )}
                      {d.status === 'confirmed' && (
                        <button className="link danger" disabled={isLoading} onClick={() => handleAction(d.id, 'refund')}>
                          {actionLoading === d.id + 'refund' ? '...' : 'Refund'}
                        </button>
                      )}
                    </td>
                  </tr>
...
/* ============================== donors page ============================== */
function DonorsPage({ token, donors, donations, organizations, loading, onRefresh }) {
  const [search, setSearch] = useState('');
  const [detailId, setDetailId] = useState(null);
  ...
  const handleDelete = async (donor) => {
    if (!window.confirm(`Delete "${donor.display_name}"? This cannot be undone.`)) return;
    try {
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      const q = search.toLowerCase();
      rows = rows.filter(d => {
        const orgName = organizations.find(o => o.id === d.organization_id)?.name || '';
        return (
    ...
    return rows.sort((a, b) => {
      ...
      if (statsA?.lastDate && statsB?.lastDate) return statsB.lastDate - statsA.lastDate;
  ...
  const totalDonatedAll = useMemo(() =>
    donations.filter(d => d.status === 'confirmed').reduce((acc, d) => acc + Number(d.amount || 0), 0)
  , [donations]);
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h1>Donors</h1>
          <p className="page-subtitle">Manage your supporter base and view giving history</p>
        </div>
      ...
      <div className="stats-row">
        ...
        <div className="stat-card">
          <div className="stat-icon purple"><Icon name="reports" size={22}/></div>
          ...
          <table className="table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} className="empty">No donors found.</td></tr>
              ) : filtered.map(d => {
                const stats = donorStats[d.id] || { totalDonated: 0, count: 0, lastDate: null };
                return (
                  <tr key={d.id}>
                    <td>
                      <div className="donor-cell">
                        <div className="donor-avatar-sm">{d.display_name?.[0] || '?'}</div>
                        <div>
                          <span className="donor-name">{d.display_name}</span>
                          {d.email && <span className="donor-org">{d.email}</span>}
                        </div>
                      </div>
                    </td>
                    <td>{organizations.find(o => o.id === d.organization_id)?.name || '—'}</td>
                    <td className="amount">{formatCurrency(stats.totalDonated)}</td>
                    <td>{stats.count}</td>
                    <td>{stats.lastDate ? formatDate(stats.lastDate) : '—'}</td>
                    <td className="actions">
                      <button className="link" onClick={() => setDetailId(d.id)}>View</button>
                      <button className="link" onClick={() => setEditDonor(d)}>Edit</button>
                      <button className="link danger" onClick={() => handleDelete(d)}>Delete</button>
                    </td>
                  </tr>
                );
              })}
      ...
      {(showCreate || editDonor) && (
        <DonorFormModal
          token={token}
          donor={editDonor}
          onClose={() => { setShowCreate(false); setEditDonor(null); }}
          onSave={onRefresh}
        />
      )}
    </div>
...
function ReceiptModal({ receipt, onClose }) {
  ...
  return (
    <Modal title={`Receipt ${receipt.receipt_number}`} onClose={onClose} size="md">
      <div className="detail-grid" style={{marginBottom: 20}}>
        <div className="detail-item"><span className="detail-label">Receipt #</span><span className="detail-value"><code>{receipt.receipt_number}</code></span></div>
        <div className="detail-item"><span className="detail-label">Amount</span><span className="detail-value">{formatCurrency(receipt.amount)}</span></div>
        <div className="detail-item"><span className="detail-label">Donor</span><span className="detail-value">{receipt.name}</span></div>
        <div className="detail-item"><span className="detail-label">Issued</span><span className="detail-value">{formatDate(receipt.issued_at)}</span></div>
        <div className="detail-item"><span className="detail-label">Donation Date</span><span className="detail-value">{formatDate(receipt.donation_date)}</span></div>
        <div className="detail-item"><span className="detail-label">Channel</span><span className="detail-value capitalize">{receipt.payment_channel || '—'}</span></div>
      </div>
...
function ReceiptsPage({ token, donations, donors, loading, onRefresh }) {
  ...
  const confirmedDonationsWithoutReceipts = useMemo(() => {
    ...
    return donations.filter(d => d.status === 'confirmed' && !receiptedIds.has(d.id));
  }, [donations, receipts]);
  ...
  return (
    <div className="page">
      ...
      {error && <div className="card error-banner" style={{marginBottom: 16, color: '#dc2626'}}>{error}</div>}

      <div className="card" style={{marginBottom: 24}}>
        <div className="table-toolbar">
          <h3 className="table-title">Ready to Receipt</h3>
          <span className="table-subtitle">{confirmedDonationsWithoutReceipts.length} confirmed donations without receipts</span>
        </div>
        <div className="table-wrap">
          <table className="table">
            <thead>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={4} className="empty">All confirmed donations have receipts.</td></tr>
                ...
                return (
                  <tr key={d.id}>
                    ...
                    <td>{formatDate(d.donation_date)}</td>
                    <td className="actions">
                      <button className="btn btn-primary btn-sm" disabled={generating === d.id} onClick={() => handleGenerate(d)}>
                        {generating === d.id ? 'Generating...' : 'Generate Receipt'}
                      </button>
                    </td>
                  </tr>
                );
          ...
          <table className="table">
            ...
            <tbody>
              {receiptsLoading ? (
                ...
                <tr><td colSpan={5} className="empty">No receipts issued yet.</td></tr>
              ) : receipts.map(r => (
                <tr key={r.id}>
                  ...
                  <td className="actions">
                    <button className="link" onClick={() => handleView(r)}>View / Print</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {detailReceipt && (
        <ReceiptModal receipt={detailReceipt} onClose={() => setDetailReceipt(null)} />
      )}
    </div>
  );
}
    ...
    <Modal title={isEdit ? 'Edit Campaign' : 'New Campaign'} onClose={onClose}>
      <form onSubmit={handleSubmit} className="form">
        ...
        <div className="form-row">
          ...
          />
          {fieldErrors.goal_amount && <span className="field-error">{fieldErrors.goal_amount[0]}</span>}
        </div>
        <div className="form-row">
        ...
        <div className="form-row">
          ...
          />
          {fieldErrors.end_date && <span className="field-error">{fieldErrors.end_date[0]}</span>}
        </div>
        <div className="form-row">
          <label>Status *</label>
...
function RaisedVsGoalChart({ campaigns }) {
  ...
  const maxValue = Math.max(...active.map(c => Math.max(Number(c.goal_amount), Number(c.raised_amount))), 1);
  ...
  return (
    <div className="hbar-chart">
      <div className="hbar-legend">
        <span className="hbar-legend-item"><i className="hbar-swatch hbar-swatch-goal" /> Goal</span>
        <span className="hbar-legend-item"><i className="hbar-swatch hbar-swatch-raised" /> Raised</span>
      </div>
      {active.map(c => {
...
function CampaignTimeline({ campaigns }) {
  ...
  const allDates = active.flatMap(c => [new Date(c.start_date), new Date(c.end_date)]);
        ...
        return (
          <div key={c.id} className="timeline-row">
            ...
            <div className="timeline-track">
              <div className="timeline-bar" style={{ marginLeft: `${offsetPct}%`, width: `${widthPct}%` }} />
            </div>
...
function CampaignsPage({ token, campaigns, loading, onRefresh }) {
  ...
  const avgCompletion = campaigns.length === 0 ? 0 : Math.round(
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h1>Campaigns</h1>
          <p className="page-subtitle">Create and track fundraising campaigns against their goals</p>
        </div>
        <button className="btn btn-primary" onClick={() => { setEditingCampaign(null); setShowForm(true); }}>
          <Icon name="plus" size={18} /> New campaign
        </button>
      </div>

      <div className="tabs">
        <button className={classNames('tab', tab === 'all' && 'active')} onClick={() => setTab('all')}>All Campaigns</button>
        <button className={classNames('tab', tab === 'performance' && 'active')} onClick={() => setTab('performance')}>Performance</button>
      </div>
            ...
            <table className="table">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr key={c.id}>
                    <td>{c.name}</td>
                    <td className="amount">{formatCurrency(c.goal_amount)}</td>
                    <td className="amount">{formatCurrency(c.raised_amount)}</td>
                    <td>{percentComplete(c)}%</td>
                    <td><CampaignStatusPill status={c.status} /></td>
                    <td>{formatDate(c.start_date)}</td>
                    <td>{formatDate(c.end_date)}</td>
                    <td className="actions">
                      <button className="link" onClick={() => { setEditingCampaign(c); setShowForm(true); }}>Edit</button>
                    </td>
                  </tr>
                ))}
              </tbody>
...
function PledgeStatusPill({ status }) {
  const map = {
    pending:             { cls: 'status-pending',   label: 'Pending' },
    partially_fulfilled: { cls: 'status-partial',   label: 'Partially Fulfilled' },
    ...
    <div className="donut-chart-wrap">
      <svg viewBox="0 0 180 180" width="180" height="180">
        <g transform="rotate(-90 90 90)">
          <circle cx="90" cy="90" r={radius} fill="none" stroke="#f59e0b" strokeWidth={stroke} />
          <circle
            cx="90" cy="90" r={radius} fill="none" stroke="#10b981" strokeWidth={stroke}
      ...
      <div className="donut-legend">
        <div className="donut-legend-item">
          <span className="donut-legend-swatch" style={{ background: '#10b981' }} />
          Fulfilled <span className="donut-legend-value">{formatCurrency(fulfilled)}</span>
        </div>
        <div className="donut-legend-item">
          <span className="donut-legend-swatch" style={{ background: '#f59e0b' }} />
          Outstanding <span className="donut-legend-value">{formatCurrency(outstanding)}</span>
        </div>
      </div>
    </div>
  );
...
/* ============================== pledge form modal ============================== */
function PledgeFormModal({ token, donors, campaigns, pledge, onClose, onSave }) {
  const isEdit = !!pledge;
  const [form, setForm] = useState({
    donor_id: pledge?.donor_id || '',
    campaign_id: pledge?.campaign_id || '',
    amount_pledged: pledge?.amount_pledged || '',
    pledge_date: pledge?.pledge_date ? pledge.pledge_date.split('T')[0] : new Date().toISOString().split('T')[0],
    ...
    <Modal title={isEdit ? 'Edit Pledge' : 'New Pledge'} onClose={onClose}>
      <form onSubmit={handleSubmit} className="form">
        <div className="form-row">
          ...
          {fieldErrors.donor_id && <span className="field-error">{fieldErrors.donor_id[0]}</span>}
        </div>
        <div className="form-row">
          <label>Campaign</label>
          <select value={form.campaign_id} onChange={e => setForm({ ...form, campaign_id: e.target.value })}>
        ...
        <div className="form-row">
          ...
          <input
            ...
            onChange={e => setForm({ ...form, amount_pledged: e.target.value })}
          />
          {fieldErrors.amount_pledged && <span className="field-error">{fieldErrors.amount_pledged[0]}</span>}
        </div>
        <div className="form-row">
        ...
        <div className="form-actions">
          <button type="button" className="btn btn-secondary" onClick={onClose}>Cancel</button>
          <button type="submit" className="btn btn-primary" disabled={saving}>{saving ? 'Saving...' : 'Save Pledge'}</button>
        </div>
      </form>
    </Modal>
  );
...
function PledgeDetailModal({ pledge, donations, onClose }) {
  ...
  const outstanding = Math.max(Number(pledge.amount_pledged) - Number(pledge.amount_fulfilled), 0);
    ...
    <Modal title="Pledge Details" onClose={onClose} size="lg">
      <div className="detail-grid">
        ...
        <div className="detail-item">
          ...
          <span className="detail-value">{formatCurrency(pledge.amount_pledged)}</span>
        </div>
        <div className="detail-item">
          <span className="detail-label">Amount Fulfilled</span>
          <span className="detail-value">{formatCurrency(pledge.amount_fulfilled)}</span>
        </div>
        <div className="detail-item">
        ...
        <div className="detail-item">
          <span className="detail-label">Status</span>
          <span className="detail-value"><PledgeStatusPill status={pledge.status} /></span>
        </div>
        <div className="detail-item">
        ...
        <div className="detail-item">
          ...
          <span className={classNames('detail-value', pledge.is_overdue && 'overdue-text')}>
      ...
      {linkedDonations.length === 0 ? (
        <p className="empty-text">No donations recorded against this pledge yet.</p>
      ) : (
        <table className="table modal-table">
          <thead>
            <tr>
...
/* ============================== pledges page ============================== */
function PledgesPage({ token, donors, campaigns, pledges, donations, summary, loading, onRefresh }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (campaignFilter) rows = rows.filter(p => p.campaign_id === campaignFilter);
  ...
  const overdueCount = summary?.overdue_count ?? pledges.filter(p => p.is_overdue).length;
  const totalOutstanding = summary?.total_outstanding ?? pledges.reduce(
    (acc, p) => acc + Math.max(Number(p.amount_pledged) - Number(p.amount_fulfilled), 0), 0
  );
  const totalFulfilled = summary?.total_fulfilled ?? pledges.reduce(
    (acc, p) => acc + Number(p.amount_fulfilled), 0
  );

  const handleDelete = async (pledge) => {
    if (!window.confirm(`Delete the pledge from ${pledge.donor_name}? This cannot be undone.`)) return;
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h1>Pledges</h1>
          <p className="page-subtitle">Track donor commitments separately from received funds</p>
        </div>
        <button className="btn btn-primary" onClick={() => { setEditingPledge(null); setShowForm(true); }}>
          <Icon name="plus" size={18} /> New pledge
        </button>
      </div>

      <div className="tabs">
        <button className={classNames('tab', tab === 'all' && 'active')} onClick={() => setTab('all')}>All Pledges</button>
        <button className={classNames('tab', tab === 'overdue' && 'active')} onClick={() => setTab('overdue')}>Overdue</button>
      </div>
      ...
      <div className="stats-row">
        ...
        <div className="stat-card">
          <div className="stat-icon red"><Icon name="alert" size={22}/></div>
      ...
      <div className="card chart-card">
        ...
        <PledgeDonutChart fulfilled={totalFulfilled} outstanding={totalOutstanding} />
      </div>

      <div className="card">
        <div className="table-toolbar">
          <h3 className="table-title">{tab === 'overdue' ? 'Overdue Pledges' : 'All Pledges'}</h3>
          <div className="table-filters">
            <div className="search-box">
              <Icon name="search" size={16}/>
              <input
          ...
          <table className="table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} className="empty">No pledges found.</td></tr>
              ) : filtered.map(p => (
                <tr key={p.id}>
                  <td>{p.donor_name}</td>
                  <td>{p.campaign_name || '—'}</td>
                  <td className="amount">{formatCurrency(p.amount_pledged)}</td>
                  <td className="amount">{formatCurrency(p.amount_fulfilled)}</td>
                  <td className={classNames(p.is_overdue && 'overdue-text')}>
                    {p.due_date ? formatDate(p.due_date) : '—'}
                  </td>
                  <td><PledgeStatusPill status={p.status} /></td>
                  <td className="actions">
                    <button className="link" onClick={() => setDetailId(p.id)}>View</button>
                    <button className="link" onClick={() => { setEditingPledge(p); setShowForm(true); }}>Edit</button>
                    <button className="link danger" disabled={deleting === p.id} onClick={() => handleDelete(p)}>
...
function MonthlyFinancialChart({ data }) {
  const maxValue = useMemo(() => Math.max(...data.map(d => Math.max(d.confirmed, d.refunded, 1)), 1), [data]);
  ...
  return (
    <div className="chart-container" style={{ height: 320, padding: '16px 20px 20px' }}>
      <div style={{ display: 'flex', gap: 16, marginBottom: 12, fontSize: 12, color: 'var(--text-muted)' }}>
        <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ width: 10, height: 10, borderRadius: 2, background: '#3b82f6' }} /> Confirmed
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ width: 10, height: 10, borderRadius: 2, background: '#14b8a6' }} /> Refunded
        </span>
      </div>
      <div className="chart-bars" style={{ height: 'calc(100% - 28px)', gap: 12 }}>
        {data.map((m, i) => {
          ...
          return (
            <div key={i} className="chart-bar-wrapper" style={{ gap: 6 }}>
              <div className="chart-bar-track" style={{ height: 220, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', gap: 3, padding: '0 4px' }}>
                <div style={{ width: '38%', maxWidth: 22, height: `${cPct}%`, background: 'linear-gradient(180deg, #3b82f6, #60a5fa)', borderRadius: '6px 6px 0 0', minHeight: 4 }} title={`Confirmed: ${formatCurrency(m.confirmed)}`} />
                <div style={{ width: '38%', maxWidth: 22, height: `${rPct}%`, background: 'linear-gradient(180deg, #14b8a6, #2dd4bf)', borderRadius: '6px 6px 0 0', minHeight: 4 }} title={`Refunded: ${formatCurrency(m.refunded)}`} />
              </div>
      ...
      {top5.map(s => {
        ...
        return (
          <div key={s.donor.id} className="hbar-row">
            <span className="hbar-label" title={s.donor.display_name}>{s.donor.display_name}</span>
            <div className="hbar-track">
              <div className="hbar-raised" style={{ width: `${pct}%` }} />
            </div>
            <span className="hbar-label" style={{ textAlign: 'right', minWidth: 90, fontWeight: 600 }}>
...
function ReportsPage({ donations, donors, campaigns, organizations, loading }) {
  ...
  const monthlyData = useMemo(() => {
    ...
    for (let i = 0; i < 12; i++) {
      months.push({
        monthIndex: i,
        label: new Date(yearFilter, i, 1).toLocaleDateString('en-US', { month: 'long' }),
        shortLabel: new Date(yearFilter, i, 1).toLocaleDateString('en-US', { month: 'short' }),
        confirmed: 0,
        refunded: 0,
        net: 0,
      });
    ...
    donations.forEach(d => {
      ...
      if (d.status === 'confirmed') months[m].confirmed += Number(d.amount || 0);
      if (d.status === 'refunded') months[m].refunded += Number(d.amount || 0);
    });
    months.forEach(m => { m.net = m.confirmed - m.refunded; });
    return months;
  ...
  const filteredCampaigns = useMemo(() => {
    let rows = [...campaigns];
    if (campaignStatusFilter) rows = rows.filter(c => c.status === campaignStatusFilter);
  ...
  const donorReportStats = useMemo(() => {
    ...
    return Object.values(stats).filter(s => s.count > 0).sort((a, b) => b.totalDonated - a.totalDonated);
  }, [donors, donations]);
  ...
  const filteredDonorStats = useMemo(() => {
    ...
    return donorReportStats.filter(s => {
      const orgName = organizations.find(o => o.id === s.donor.organization_id)?.name || '';
      return (
  ...
  const handleExport = () => {
    if (tab === 'financial') {
      ...
      const rows = monthlyData.map(m => [m.label, m.confirmed, m.refunded, m.net]);
      rows.push(['Total', financialTotals.confirmed, financialTotals.refunded, financialTotals.net]);
      ...
      const headers = ['Name', 'Goal', 'Raised', '% Complete', 'Status', 'Start Date', 'End Date'];
      ...
      const headers = ['Donor', 'Organization', 'Total Donated', '# Donations', 'Average Donation', 'Last Donation'];
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h1>Reports</h1>
          <p className="page-subtitle">Aggregate financial and campaign reporting for leadership and audit</p>
        </div>
      ...
      <div className="tabs">
        <button className={classNames('tab', tab === 'financial' && 'active')} onClick={() => setTab('financial')}>
          Financial Summary
        </button>
        <button className={classNames('tab', tab === 'campaigns' && 'active')} onClick={() => setTab('campaigns')}>
          Campaign Performance
        </button>
        <button className={classNames('tab', tab === 'donors' && 'active')} onClick={() => setTab('donors')}>
          Donor Giving History
        </button>
      </div>
      ...
      {tab === 'financial' && (
        <>
          <div className="card">
            ...
            {loading ? (
              <div className="chart-empty"><p>Loading...</p></div>
            ) : (
            ...
            <div className="table-wrap">
              <table className="table">
                ...
                <tbody>
                  {loading ? (
                    <tr><td colSpan={4} className="empty">Loading...</td></tr>
                  ) : monthlyData.every(m => m.confirmed === 0 && m.refunded === 0) ? (
                    <tr><td colSpan={4} className="empty">No data for {yearFilter}.</td></tr>
                  ) : (
                    <>
                      {monthlyData.map((m, i) => (
                        <tr key={i}>
                          <td>{m.label}</td>
                          <td className="amount">{formatCurrency(m.confirmed)}</td>
                          <td className="amount">{formatCurrency(m.refunded)}</td>
                          <td className="amount" style={{ color: m.net < 0 ? 'var(--danger)' : 'inherit' }}>{formatCurrency(m.net)}</td>
                        </tr>
                      ))}
                      <tr style={{ fontWeight: 700, background: '#f8fafc' }}>
                        <td>Total</td>
                        <td className="amount">{formatCurrency(financialTotals.confirmed)}</td>
                        <td className="amount">{formatCurrency(financialTotals.refunded)}</td>
                        <td className="amount" style={{ color: financialTotals.net < 0 ? 'var(--danger)' : 'inherit' }}>{formatCurrency(financialTotals.net)}</td>
                      </tr>
                    </>
                  )}
                </tbody>
      ...
      {tab === 'campaigns' && (
        <>
          <div className="stats-row">
            <div className="stat-card">
              <div className="stat-icon blue"><Icon name="campaigns" size={22} /></div>
              <div className="stat-value">{campaignReportStats.count}</div>
              <div className="stat-label">Total Campaigns</div>
            </div>
            <div className="stat-card">
              <div className="stat-icon green"><Icon name="receipts" size={22} /></div>
              <div className="stat-value">{formatCurrency(campaignReportStats.totalRaised)}</div>
              <div className="stat-label">Total Raised</div>
            </div>
            <div className="stat-card">
              <div className="stat-icon purple"><Icon name="reports" size={22} /></div>
              <div className="stat-value">{campaignReportStats.avgCompletion}%</div>
      ...
      {tab === 'donors' && (
        <>
          <div className="stats-row">
            ...
            <div className="stat-card">
              <div className="stat-icon green"><Icon name="receipts" size={22} /></div>
              <div className="stat-value">{formatCurrency(totalConfirmedAll)}</div>
              <div className="stat-label">Total Confirmed Donations</div>
            </div>
            <div className="stat-card">
              <div className="stat-icon purple"><Icon name="reports" size={22} /></div>
              ...
              <table className="table">
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr><td colSpan={6} className="empty">No donor data available.</td></tr>
                  ) : filteredDonorStats.map(s => (
                    <tr key={s.donor.id}>
                      <td>
                        <div className="donor-cell">
                          <span className="donor-name">{s.donor.display_name}</span>
                          {s.donor.email && <span className="donor-org">{s.donor.email}</span>}
                        </div>
                      </td>
                      <td>{organizations.find(o => o.id === s.donor.organization_id)?.name || '—'}</td>
                      <td className="amount">{formatCurrency(s.totalDonated)}</td>
                      <td>{s.count}</td>
                      <td className="amount">{formatCurrency(s.totalDonated / s.count)}</td>
                      <td>{s.lastDate ? formatDate(s.lastDate.toISOString()) : '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function RolePermissionMatrixModal({ token, role, onClose, onSaved }) {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    api.getRolePermissions(token, role)
  ...
  return (
    <Modal title={`Permissions — ${role}`} onClose={onClose} size="md">
      {loading ? (
        ...
        <>
          <div className="detail-list" style={{ maxHeight: 360, overflowY: 'auto' }}>
            {permissions.map(p => (
              <label key={p.permission} className="detail-row" style={{ cursor: 'pointer' }}>
                <span className="detail-label"><code>{p.permission}</code></span>
                <input
                  type="checkbox"
                  checked={p.granted}
...
function SettingsPage({ token }) {
  ...
  const loadReceiptSettings = useCallback(() => {
    ...
    api.getReceiptSettings(token)
      .then(res => { setReceiptSettings(res); setReceiptPrefix(res?.prefix || ''); })
      .catch(err => setReceiptError(err.message))
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h1>Settings</h1>
          <p className="page-subtitle">Roles & permissions, payment channels, and receipt numbering for your organization</p>
        </div>
      </div>

      <div className="tabs">
        <button className={classNames('tab', tab === 'roles' && 'active')} onClick={() => setTab('roles')}>
          Roles & Permissions
        </button>
        <button className={classNames('tab', tab === 'channels' && 'active')} onClick={() => setTab('channels')}>
          Payment Channels
        </button>
        <button className={classNames('tab', tab === 'receipt' && 'active')} onClick={() => setTab('receipt')}>
          Receipt Numbering
        </button>
      </div>
            ...
            <table className="table">
              ...
              <tbody>
                {rolesLoading ? (
                  ...
                  <tr><td colSpan={4} className="empty">No roles found.</td></tr>
                ) : roles.map(r => (
                  <tr key={r.role}>
                    <td className="capitalize">{r.role.replace(/_/g, ' ')}</td>
                    <td>{r.permission_count}</td>
                    <td>{r.last_modified ? formatDate(r.last_modified) : '—'}</td>
                    <td>
                      <button className="btn btn-ghost" onClick={() => setEditingRole(r.role)}>
                        View / Edit
                      </button>
                    </td>
                  </tr>
      ...
      {tab === 'channels' && (
        <div className="card">
          <div className="table-toolbar">
            ...
            <button className="btn btn-primary" onClick={saveChannels} disabled={channelsSaving || channelsLoading}>
          ...
          <div className="detail-list">
            {channelsLoading ? (
              ...
              <div key={c.channel} className="detail-row">
                <span className="detail-label capitalize">{c.channel.replace(/_/g, ' ')}</span>
            ...
            <form onSubmit={saveReceiptPrefix}>
              ...
              <div className="detail-row">
                ...
                <span className="detail-value text-muted">
                  {receiptSettings?.next_sequence ?? '—'} (server-generated, not editable)
                </span>
              </div>
...
export default function App() {
  ...
  const fetchAll = useCallback(async (currentToken) => {
    ...
    return (
      <div className="login-screen">
        <div className="login-card">
          <div className="brand-logo-lg"><Icon name="donations" size={32}/></div>
          ...
          <input
            ...
            autoFocus
          />
          <p className="login-hint">Run <code>npm run seed</code> in your backend to generate a token.</p>
        </div>
      </div>
    );
  }
  ...
  />
          ...
          {page !== 'dashboard' && page !== 'donations' && page !== 'campaigns' && page !== 'pledges' && page !== 'donors' && page !== 'organizations' && page !== 'receipts' && page !== 'reports' && page !== 'settings' && (
            
            <div className="page">
              <div className="card empty-state">
                <h2>Coming Soon</h2>
                <p>This module hasn't been built yet. Switch to Donations or Donors to continue.</p>
                <button className="btn btn-primary" onClick={() => setPage('donations')}>
                  Go to Donations
                </button>
              </div>
            </div>
