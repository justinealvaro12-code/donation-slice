...
async function ensureDefaultsForOrg(organizationId, userId) {
  for (const role of ALL_ROLES) {
    const existing = await pool.query(
      `SELECT 1 FROM organization_role_permissions WHERE organization_id = $1 AND role = $2 LIMIT 1`,
      [organizationId, role]
    );
    ...
    for (const permission of defaults) {
      await pool.query(
        `INSERT INTO organization_role_permissions (organization_id, role, permission, created_by, updated_by)
         VALUES ($1, $2, $3, $4, $4)
...
async function getRolePermissionMatrix(organizationId, role, userId) {
  ...
  const result = await pool.query(
    `SELECT permission FROM organization_role_permissions WHERE organization_id = $1 AND role = $2`,
    [organizationId, role]
  );
...
async function updateRolePermissions(organizationId, role, permissions, userId) {
  ...
  try {
    ...
    await client.query(
      `DELETE FROM organization_role_permissions WHERE organization_id = $1 AND role = $2`,
      [organizationId, role]
    );
...
async function getPaymentChannels(organizationId) {
  const result = await pool.query(
    `SELECT channel, is_active FROM organization_payment_channels WHERE organization_id = $1`,
    [organizationId]
  );
  const byChannel = {};
...
async function updatePaymentChannels(organizationId, channels, userId) {
  for (const { channel, is_active } of channels) {
    ...
    await pool.query(
      `INSERT INTO organization_payment_channels (organization_id, channel, is_active, updated_by)
       VALUES ($1, $2, $3, $4)
...
async function isChannelActive(organizationId, channel) {
  const result = await pool.query(
    `SELECT is_active FROM organization_payment_channels WHERE organization_id = $1 AND channel = $2`,
...
async function getReceiptSettings(organizationId) {
  const result = await pool.query(
    `SELECT prefix, next_sequence FROM organization_receipt_settings WHERE organization_id = $1`,
    [organizationId]
  );
...
async function updateReceiptPrefix(organizationId, prefix, userId) {
  ...
  const result = await pool.query(
    `UPDATE organization_receipt_settings SET prefix = $1, updated_by = $2, updated_at = now()
...
async function getAndIncrementSequence(client, organizationId) {
  ...
  const result = await client.query(
    `SELECT prefix, next_sequence FROM organization_receipt_settings
  ...
  await client.query(
    `UPDATE organization_receipt_settings SET next_sequence = next_sequence + 1, updated_at = now()
     WHERE organization_id = $1`,
    [organizationId]
  );
